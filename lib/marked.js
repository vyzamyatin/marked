...
Lexer.prototype.lex = function(src) {
  ...
  return this.token(src, true, false, []);
};
...
Lexer.prototype.token = function(src, top, bq, links) {
  ...
  while (src) {
    ...
    if (cap = this.rules.blockquote.exec(src)) {
      ...
      tokens = tokens.concat(this.token(cap, top, true, links).tokens);
    ...
    if (cap = this.rules.list.exec(src)) {
      ...
      for (; i < l; i++) {
        ...
        // Recurse.
        tokens = tokens.concat(this.token(item, false, bq, links).tokens);
...
inline.gfm = merge({}, inline.normal, {
  ...
  url: /^((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s`!()\[\]{};:'".,<>?«»“”‘’]|[`!\[\]{};:'".,<>?«»“”‘’](?=[^\s()<>])|\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\))+)/i,
  del: /^~~(?=\S)([\s\S]*?\S)~~/,
...
function splitOnPipes(str) {
  ...
  // @hmhealey This assumes that there are no leading pipes on the string
